<div class="w-full h-52 relative flex items-center justify-center header-bg" style="background: url({{ setting('cms_header') }}) no-repeat center center; background-size: cover">
        ...
        <div class="max-w-7xl relative h-full w-full flex items-center justify-center md:justify-between pr-10">
            <div class="flex gap-x-4 items-center">
                ...
                <div class="hidden md:flex items-center bg-white dark:bg-gray-900 dark:text-white px-4 rounded-md relative h-[50px]">
                    ...
                    <span class="relative">
                        {{ __(':online :hotel online', ['online' => DB::table('users')->where('online', '1')->count(), 'hotel' => setting('guests_name')]) }}
                    </span>
                </div>
            </div>
...
{{--                @if(config('habbo.client.flash_enabled') && false)--}}
